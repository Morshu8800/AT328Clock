...
void control(void) {
  ...
  void runningText(void) {
    ...
    oled.setCursor(0, 2);
    oled.print("MORSHU И ALEXGYVER");
    oled.setCursor(0, 4);
    oled.print("ВЕРСИЯ 1.0,(С)2024");
    oled.setCursor(0, 6);
  ...
  void savebat(void) {
    if (millis() - savebattime > 150000 and timedisp==1 and settup==0 and setings_alarm==0 and setings_time==0) {
      ...
      if (but.holding() or jy.value() <= -100 or jy.value() >= 100 or jx.value() <= -100 or jx.value() >= 100 or timedisp==0 and settup==1 and setings_alarm==1 and setings_time==1) {
        oled.setPower(true);
        savebattime = millis();
        sp = 1;
      }
